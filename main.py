...
from src.tools.mock_news_api import (  # For sequential comparison
...
async def main() -> None:
    ...
    print(
        f"Agent Config Loaded: max_cpu_workers={agent_config.max_cpu_workers}, "
    ...
    async with ExecutorManager(
        max_workers=agent_config.max_cpu_workers
    ) as executor_manager:
        agent = NewsArticleAgent(
            agent_config=agent_config, executor_manager=executor_manager
        )
        ...
        try:
            ...
            print(
                f"Total time taken (Hybrid Pattern): {report.total_time_taken:.2f} seconds"
            )
            for result in report.results:
                print(f"  URL: {result.url}")
                print(f"    Category: {result.category}")
                print(
                    f"    Summary: {result.summary[:70]}..."
                )  # Print first 70 chars of summary
            ...
            print(
                "Note: This is a simplified sequential simulation within one process."
            )
            for url in mock_article_urls:
                raw_data = await async_mock_fetch_article_content(
                    url, agent_config.mock_fetch_delay
                )
                analysis_result = cpu_mock_analyze_article_text(
                    raw_data, agent_config.mock_analysis_time
                )
            ...
            print(
                f"Sequential processing for {len(mock_article_urls)} articles: {sequential_duration:.2f} seconds"
            )
            print(
                f"Hybrid pattern achieved a speedup of: {sequential_duration / report.total_time_taken:.2f}x"
            )
